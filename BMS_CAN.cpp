...
BMS_CAN::BMS_CAN(uint8_t csPin, long baudrate) : mcp(csPin), baudrate(baudrate) {
    soc = 0.0;
    batteryVoltage = 0.0;
    batteryCurrent = 0.0;
...
void BMS_CAN::parsePacket(uint32_t id, uint8_t *data, uint8_t len) {
    if (id == 0x18FF28F4) { // BMS Basic Information 1
        ...
        uint32_t newBatteryCycle = (data[3] << 24) | (data[2] << 16) | (data[1] << 8) | data[0];
        uint32_t newEnergyCycle = (data[7] << 24) | (data[6] << 16) | (data[5] << 8) | data[4];
